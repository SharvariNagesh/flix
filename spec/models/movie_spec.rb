...
describe "A Movie" do
   ...
   it "should reply true if total movie gross is less than $50M" do 

   	movie = Movie.new(total_gross:40000)
   ...
   it "should reply false if movie total_gross > $50M " do
   
   	movie = Movie.new(total_gross:500000000)
   
   	expect(movie.flop?).to eq(false)
   
   end

   it "should include movie released 3 months ago" do
   	movie = Movie.create(movie_attributes(released_on: 3.month.ago))

   	expect(Movie.released).to include(movie)

   end

   it "should not include movie, which is not released" do
...
#These test cases are related to review-movie relations
